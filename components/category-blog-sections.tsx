    ...
    <div className="grid gap-4">
      <div className="grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="rounded-md border border-fp-line bg-fp-soft p-4">
          ...
          <a
            className="mt-8 inline-flex items-center gap-2 text-sm font-extrabold text-fp-green"
      ...
      <div className="divide-y divide-fp-line rounded-md border border-fp-line/60 bg-fp-soft">
        {listPosts.map((post, index) => (
          <a
            ...
            className="grid gap-3 p-4 sm:grid-cols-[5.5rem_1fr]"
          >
            ...
            <span className="min-w-0">
              ...
              <span className="mt-1 block text-base font-extrabold leading-tight text-fp-ink">
                {post.title}
              </span>
...
function WideRowLayout({ posts }: { posts: Article[] }) {
  ...
  return (
    <div className="grid gap-4">
      <a
        href={getArticleHref(lead)}
        className="grid overflow-hidden rounded-md border border-fp-line bg-fp-soft lg:grid-cols-[18rem_1fr_auto]"
      >
        ...
        <span className="p-4 sm:p-5">
          ...
          <span className="mt-2 block text-2xl font-bold leading-tight text-fp-ink">
            {lead.title}
          </span>
...
function OverlayPost({ post, className = "" }: { post: Article; className?: string }) {
  return (
    <a
      href={getArticleHref(post)}
      className={`group relative block overflow-hidden rounded-md bg-fp-ink ${className}`}
    >
      <Image
        src={post.image}
        alt={post.title}
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 p-3">
        ...
        <span className="mt-1 line-clamp-2 block text-base font-extrabold leading-tight text-fp-ink">
          {post.title}
        </span>
...
function PostMeta({ post, className = "" }: { post: Article; className?: string }) {
  return (
    <span
      className={`flex flex-wrap items-center gap-2 text-xs font-semibold text-fp-muted ${className}`}
    >
      <span>{post.meta}</span>
