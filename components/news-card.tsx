...
export function NewsCard({ card }: { card: MediaCard }) {
  ...
  return (
    <article data-clickable="true" className="min-w-[155px] overflow-hidden rounded-md border border-fp-line bg-white p-3 shadow-card sm:min-w-0">
      <div className="relative h-28 overflow-hidden rounded-sm sm:h-36">
        <Image src={card.image} alt={card.title} fill className="object-cover" sizes="(max-width: 640px) 155px, 33vw" />
