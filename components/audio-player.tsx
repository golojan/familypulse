...
export function AudioPlayer({ episode, index = 0 }: AudioPlayerProps) {
  return (
    <div
      data-clickable="true"
      className="grid grid-cols-[78px_1fr_auto] items-center gap-3 border-b border-fp-line px-3 py-3 last:border-0 sm:grid-cols-[96px_1fr_auto] sm:px-4"
    >
      ...
      <div className="min-w-0">
        <h3 className="truncate text-sm font-extrabold text-fp-ink sm:text-base">
          {episode.title}
        </h3>
      ...
      <span className="rounded-full bg-fp-mint px-2.5 py-1 text-xs font-extrabold text-fp-green sm:text-sm">
        {episode.time}
      </span>
    </div>
  );
